...
export type HeroProps = { slides: { title: string; image: any }[] };
    ...
    <section id="hero">
      <div className="bg-gray-100" style={{ height: '50vh' }}>
        <div className="relative w-full h-full">
          {slides.map(({ title }, index: number) => {
            return (
              <div
                key={`image-${index}`}
                className={`absolute bg-cover bg-center bg-gray-900 w-full h-full transition-all duration-500 ${
                  activeIndex === index ? 'z-20' : 'z-10'
                }`}
                style={{
                  backgroundImage: `url("/images/hero/hero0${index + 1}.jpeg")`,
                }}
              >
